...
def forwardBackward(AllObs, V, Q, Amat, Bmat):
    ...
    while notConverged(count):
        ...
        for Obs in AllObs[:int(len(AllObs)/10)]:
            ...
            if ObsCount%50==0:
                print(ObsCount)
            alpha, alphaSum = forward(A, B, Q, Obs, pi)
            ...
            for t in range(T):
                for j in Q:
                    numerator = alpha[t][j]*beta[t][i]
                    sumalpha= 0
                    for thing in states:
                        denomenator += alpha[T-1][j]

                    gamma[t][j] =numerator/sumalpha
                    for i in Q:
                        if t < T-1:
                            numerator = alpha[t][i]*A[(i,j)]*beta[t+1][j]*b[(obs[t+1],j)]
                            denomenator= sumalpha
                            zeta[t][(i,j)] = numerator/denomenator
